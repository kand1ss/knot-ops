...
pub struct Message<Req, Res, Ev> {
    ...
    /// For events, this can optionally be used to correlate the event with a specific request.
    pub id: u32,
    ...
    Ev: Serialize,
{
    ...
    ///
    /// Events are one-way notifications. They can be associated with an ongoing
    /// request-response cycle by passing the corresponding `id`, or be unassociated.
    ///
    /// # Arguments
...
mod messages_tests {
    ...
    fn test_event_id() {
        ...
        assert_eq!(msg.id, 0, "event ID should match the provided ID");
    }
